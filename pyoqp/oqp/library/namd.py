...
_P_RESCALE = 13         # 0 isotropic; 1 analytic derivative-coupling direction
_NPARAMS = 16
...
class NAMD:
    ...
    def _write_md_trajectory(self, istep, coordinates, epot, ekin, hopped):
        ...
        for field in (
                ...
                'nve_metrics', 'rescale_gamma', 'rescale_discriminant',
    ...
    def _load_restart(self):
        ...
        with np.load(self.restart_file, allow_pickle=False) as saved:
            ...
            for name in ('ba_energy_left', 'ba_energy_center', 'ba_tdc_left',
                         'ba_dt_left', 'analytic_tdc_previous',
                         'nve_reference_energy',
                         'nve_previous_energy'):
                marker = f'has_{name}'
                value = (np.array(saved[name], copy=True)
                         if marker in saved and int(saved[marker][0]) else None)
