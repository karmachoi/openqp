...
!   Compute xc gradient
    if (dft) then
      ...
      ! The relaxed MRSF density is a linear XC probe.  Add the part of its
      ! derivative that comes only from moving atom-centred points and the
      ! normalized fuzzy-cell weights.  The ordinary call above must retain
      ! the established ground-state contribution; requesting moving-grid
      ! response in that same call would also translate the ground-state AO
      ! term without its matching ground-state partition derivative.  This
      ! correction-only resident sweep therefore uses P=T+Z with the ground
      ! state excluded and returns only the moving-grid contribution.
      block
        ...
        ! utddft_xc_gradient scales and unscales its density arguments in
        ! place.  Copies keep a second normalization round trip from changing
        ! the p/d arrays subsequently consumed by mrsf_2e_grad.
        grid_d = d
        grid_p = p
        call utddft_xc_gradient(basis=basis, &
             molGrid=molGrid, &
