...
class AnalyticHessianExternalRuntimeTests(unittest.TestCase):
    ...
    def test_pyscf_hessian_is_flattened_and_symmetrized_for_openqp(self):
        ...
        self.assertEqual(metadata["shape"], [6, 6])
        self.assertAlmostEqual(metadata["max_asymmetry"], 2.0)
        compact_summary = metadata["compact_validation_summary"]
        self.assertEqual(compact_summary["schema_version"], "analytic_hessian_bridge.v1")
        self.assertEqual(compact_summary["report_type"], "runtime_bridge_metadata")
        self.assertEqual(compact_summary["matrix_payload"], "omitted")
        self.assertEqual(compact_summary["shape"], [6, 6])
        self.assertAlmostEqual(compact_summary["max_asymmetry"], 2.0)
        self.assertTrue(compact_summary["symmetrized"])
        self.assertEqual(compact_summary["backend"], "external_pyscf")
        self.assertEqual(compact_summary["method"], "hf")
        self.assertEqual(compact_summary["scf_type"], "rhf")
        self.assertEqual(compact_summary["functional"], "")
        self.assertEqual(compact_summary["basis"], "unknown")
        self.assertEqual(compact_summary["state"], 0)
        self.assertEqual(compact_summary["molecule"], "h2")
...
class AnalyticHessianNativeDispatchTests(unittest.TestCase):
    ...
    def test_native_hf_hessian_rejects_nonfinite_output_without_pyscf_fallback(self):
        ...
        bad_hessian[0, 5] = np.nan
        hessian = self.single_point.Hessian(mol)
        hessian.native_hess_func["hf"] = lambda target: target.set_hessian_result(bad_hessian)
    ...
    def test_native_hf_hessian_forces_audit_metadata(self):
        ...
        raw = np.eye(6)
        hessian = self.single_point.Hessian(mol)
        hessian.native_hess_func["hf"] = lambda target: target.set_hessian_result(raw)
