...
class TestMrsfReferenceScfWiring(unittest.TestCase):
    ...
    def test_python_runs_block_response_before_legacy_guard(self):
        ...
        self.assertIn("collect_block_diagonal_response(", source)
        self.assertIn("collect_state_interaction_response(", source)
        self.assertIn("'state_interaction_diagnostic': state_interaction", source)
        ...
        self.assertIn("final_response = block_diagonal", source)
        self.assertIn("final_response.get('model', 'block_diagonal_floored')", source)
