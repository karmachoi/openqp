...
\begin{table}[t]
\caption{\ce{H2} \emph{ground-state} total energy at $R=1.4\,a_0$ in aug-cc-pVDZ.
HF, MP2, and the in-basis FCI are computed with \textsc{PySCF} and reproduced to all
printed digits by an independent from-scratch code (cross-check). ``TC-FCI'' is full
configuration interaction with the Boys--Handy transcorrelated Hamiltonian; the exact
value is the Kolos--Wolniewicz Born--Oppenheimer energy. These validate the
transcorrelated \emph{operator} at the full-CI level. They are ground-state energies
only---no MRSF-CIS or excitation-energy values are reported (see
Sec.~\ref{sec:status}).}
\label{tab:h2}
\begin{ruledtabular}
\begin{tabular}{lc}
...
HF                                   & $-1.128788$ \\
MP2                                  & $-1.156078$ \\
FCI (in-basis)                       & $-1.164608$ \\
Boys--Handy TC-FCI ($\gamma{=}0.1$)  & $-1.171621$ \\
\colrule
exact (Kolos--Wolniewicz)            & $-1.174475$ \\
\end{tabular}
\end{ruledtabular}
\end{table}

\paragraph{Status of the excited-state (MRSF) method.}\label{sec:status}
The results in this section validate the transcorrelated \emph{operator} and its
integrals at the full-CI level---not the excited-state response. The excited-state
method itself, TC--MRSF-CIS and its projective (pTC) variant, requires building
$\Hbar$ into the MRSF response and solving the resulting \emph{non-Hermitian}
eigenproblem with a complex/non-symmetric Davidson; that integration into OpenQP is
in progress and \emph{no} TC--MRSF-CIS excitation energies are reported here. We also
note an implementation constraint of the host code: its MRSF-CIS requires a
closed-shell reference, so \ce{H2}---whose high-spin reference has no doubly-occupied
orbital---is treated at the FCI level above, and the MRSF benchmarks will use
