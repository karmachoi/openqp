...
solver returns the exact spectrum; \ce{H4}/STO-3G, where
$\bra{\mathrm{HF}}\Hbar\ket{\mathrm{HF}}=E_{\mathrm{MP2}}$ to machine precision and
the transcorrelated compact solve recovers $62\%$ of the full-CI correlation
energy; and \ce{H2} dissociation, where pTC--MRSF-CIS recovers the dynamic
correlation that bare MRSF-CIS misses for the singlet states while leaving the
triplet undressed---making explicit the need for the spin-resolved cusp. We argue that
...
\caption{\ce{H2} dissociation in a 6-311G ($s$-only) basis: bare MRSF-CIS in the
$(2,2)$ frontier space (dashed), pTC--MRSF-CIS with the same compact space (dotted),
and full configuration interaction in the six-orbital space (solid), for the
ground singlet $S_0$, the lowest triplet $T_1$, and the lowest open-shell singlet
$S_1$. For $S_0$ the three curves are distinct: bare MRSF-CIS misses the dynamic
correlation carried by the four orbitals outside the active space, and
pTC--MRSF-CIS recovers $\sim70\%$ of it near equilibrium (closing toward FCI),
the recovery decreasing toward dissociation as the closed-shell MP2 correlator
used here (a placeholder for the full pTC geminal) degrades with the breaking
reference. The bare and pTC triplet curves \emph{coincide}: the closed-shell
correlator is a singlet-pair operator and does not dress $T_1$---precisely the
asymmetry that Ten-no's spin-resolved geminal, fixing the singlet \emph{and}
triplet cusps simultaneously, is designed to remove. All energies are absolute
Hartrees; $S_0$ and $T_1$ correctly become degenerate as \ce{H2}$\to$2\,H.}
\label{fig:h2-pes}
\end{figure*}

\paragraph{Molecular correlation recovery (\ce{H4}/STO-3G).}
On the \ce{H4} chain in a minimal STO-3G basis---a four-electron system with
genuine dynamic correlation outside the frontier space---the entire pipeline is
run from native integrals. The one-electron part is anchored independently: the
\ce{H2}/STO-3G restricted Hartree--Fock energy is $-1.1167$~Ha, matching the
textbook value. Building the transcorrelated Hamiltonian and evaluating it in the
Hartree--Fock determinant reproduces the second-order M{\o}ller--Plesset energy
\emph{exactly}, $\bra{\mathrm{HF}}\Hbar\ket{\mathrm{HF}}=E_{\mathrm{MP2}}$ to
$4\times10^{-16}$~Ha---the rigorous check that $\Hbar$ is constructed correctly,
since for an excitation-only correlator $\bra{\mathrm{HF}}\ee^{-\tau}=\bra{\mathrm{HF}}$
and $\bra{\mathrm{HF}}\Hbar\ket{\mathrm{HF}}=E_{\mathrm{HF}}+E_{\mathrm{MP2}}^{(2)}$.
The bare compact (Hartree--Fock-plus-singles) ground energy equals $E_{\mathrm{HF}}$
(Brillouin's theorem), whereas the transcorrelated compact solve recovers $61.9\%$
of the full-CI correlation energy. With the transcorrelation switched off
($\tau=0$) the non-Hermitian solve reproduces the bare result bit-for-bit, the
acceptance gate for the whole construction.
