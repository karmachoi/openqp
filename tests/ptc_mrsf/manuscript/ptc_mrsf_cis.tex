...
density-fitted. The transcorrelated response matrix is non-Hermitian, requiring a general
(non-symmetric) reduced-space eigensolver that reduces to the standard symmetric
solver in the $\tau=0$ limit. As a proof of concept for the transcorrelated
\emph{operator}, a self-contained, package-free implementation shows for \ce{H2}
that full configuration interaction with $\Hbar$ (TC-FCI) drives the finite-basis
energy down toward the exact Kolos--Wolniewicz limit---recovering basis-set
incompleteness that the same wavefunction misses under the bare Hamiltonian---while
the bare HF, MP2, and FCI energies reproduce an independent \textsc{PySCF}
calculation to all printed digits and the transcorrelated integrals are certified to
$\sim\!10^{-12}$ against analytic and operator-identity targets. Building $\Hbar$
into the MRSF excited-state response (the non-Hermitian eigensolver plus
transcorrelated effective integrals) is in progress and is not benchmarked here. We argue that
...
% [Figure removed: the H2 dissociation PES previously shown here plotted
%  "bare MRSF-CIS" and "TC-MRSF-CIS" curves produced by a standalone prototype CI,
%  not OpenQP's validated MRSF (which cannot run H2: its high-spin reference has no
%  doubly-occupied orbital). Those prototype MRSF curves are withdrawn. A real
%  TC-MRSF-CIS figure will be added once H_bar is built into OpenQP's MRSF response.]

\begin{table}[t]
\caption{\ce{H2} at $R=1.4\,a_0$: ground-state total energy $E(S_0)$ and the lowest
excitation energies. \emph{bare MRSF-CIS} is the real host-code (OpenQP) MRSF-CIS
---MRSF-TDDFT at $100\%$ Hartree--Fock exchange with no functional---in aug-cc-pVDZ.
\emph{TC-FCI} is full configuration interaction with the Boys--Handy transcorrelated
Hamiltonian (aug-cc-pVDZ, geminal exponent $\gamma{=}0.1$). HF, MP2, FCI, and ADC(2)
are independent \textsc{PySCF} values (aug-cc-pVDZ; the bare HF/MP2/FCI are reproduced
to all printed digits by an independent from-scratch code). MP2(CBS) is a $T/Q$
basis-set-limit extrapolation; the exact $S_0$ is the Kolos--Wolniewicz value. Ground
states in Ha, excitations in eV. Boys--Handy TC-FCI recovers basis-set incompleteness
toward the CBS/exact limit; the projective transcorrelation (pTC) and the combination
of either transcorrelation with MRSF-CIS (TC/pTC--MRSF-CIS) are in development
(Sec.~\ref{sec:status}).}
\label{tab:h2}
\begin{ruledtabular}
\begin{tabular}{lccc}
method & $E(S_0)$ (Ha) & $T_1{-}S_0$ & $S_1{-}S_0$ \\
\colrule
HF                       & $-1.128788$ & --       & --       \\
MP2                      & $-1.156078$ & --       & --       \\
bare MRSF-CIS            & $-1.115079$ & $9.379$  & $11.455$ \\
ADC(2)                   & --          & $10.423$ & $12.687$ \\
FCI (in-basis)           & $-1.164608$ & $10.509$ & $12.654$ \\
Boys--Handy TC-FCI       & $-1.171621$ & --       & --       \\
MP2 (CBS, $T/Q$ extrap.) & $-1.167786$ & --       & --       \\
\colrule
exact (Kolos--Wolniewicz, CBS) & $-1.174475$ & --  & --       \\
\end{tabular}
\end{ruledtabular}
\end{table}

\paragraph{Status: what is and is not yet validated.}\label{sec:status}
Three things are established with verified, reproducible numbers (Table~\ref{tab:h2}):
(i) the bare host-code \emph{MRSF-CIS}, now running for \ce{H2} after a fix to the
MRSF response for references with no doubly-occupied core; (ii) the \emph{Boys--Handy}
transcorrelated operator at the full-CI level (TC-FCI), which recovers basis-set
incompleteness toward the CBS/exact limit; and (iii) the bare HF/MP2/FCI energies,
cross-checked to all digits against an independent package. Two things are
\emph{not} yet validated and no numbers are reported for them. First, the
\emph{projective} transcorrelation (pTC): its second-order energy should equal the
MP2 basis-set limit, but the present integral assembly recovers only part of that
limit ($\sim\!2.7$ of $6.9$~m$E_\mathrm{h}$ for \ce{H2}/aug-cc-pVDZ), so the pTC
effective-integral construction is still being corrected against that target. Second,
the combination of either transcorrelation with the excited-state response,
TC/pTC--MRSF-CIS, which requires building $\Hbar$ into the MRSF matrix-vector product
and solving the resulting \emph{non-Hermitian} eigenproblem; the non-symmetric
reduced-space solver is in place and reproduces bare MRSF-CIS at $\tau=0$, but the
transcorrelated effective integrals are not yet injected, so no TC/pTC--MRSF-CIS
excitation energies are reported.
...
($\op{D}$ the double-occupancy operator) at the optimal $g$ makes the mean-field
determinant the exact right-eigenvector of $\Hbar$, $\Hbar\ket{g^2}=E_0\ket{g^2}$
(an analytic identity for the two-site model), so it recovers the full correlation
energy. This is the mechanism TC--MRSF-CIS exploits: a compact (CIS-like) reference
