...
reference, computed by the genuine pTC--MRSF-CIS (ROHF reference plus the
...
error (mean $0.22\!\to\!0.18$~eV against FCI). \emph{Right}: the per-state pTC
correction $E_{\mathrm{bare}}-E_{\mathrm{pTC}}$; both the singlets \emph{and the
...
minimal test: as a CIS-cost method pTC--MRSF-CIS lies above the in-basis FCI total
...
This is the mechanism pTC--MRSF-CIS exploits: a compact (CIS-like) reference
